...
contract Accounts {
    ...
    function get(
        address account
    ) internal view returns (Types.AccountDetails memory) {
        require(account != address(0));
    ...
    function addparty(
        Types.AccountDetails memory account,
        address myAccount
    ) internal {
    ...
    function getMyPartyList(
        address user_id
    ) internal view returns (Types.AccountDetails[] memory accountsList_) {
